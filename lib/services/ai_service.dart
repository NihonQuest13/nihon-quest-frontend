...
class AIService {
  ...
  static Future<String> updateRoadMap(Novel novel) async {
    ...
    debugPrint("Appel à updateRoadMap (génération du résumé PASSÉ) pour ${novel.title} ($chapterCount chapitres)...");

    List<Chapter> relevantChapters;
    if (chapterCount < 3) {
      debugPrint("Moins de 3 chapitres, impossible de générer la roadmap.");
      return novel.roadMap ?? "Pas assez de chapitres pour générer un résumé.";
    } else if (chapterCount == 3) {
      relevantChapters = novel.chapters;
      debugPrint("Utilisation des 3 premiers chapitres pour la création initiale de la roadmap.");
    } else {
      relevantChapters = novel.chapters.sublist(chapterCount - 3);
       debugPrint("Utilisation des 3 derniers chapitres (indices ${chapterCount - 3} à ${chapterCount - 1}) pour la mise à jour de la roadmap.");
    }
    ...
    debugPrint("Envoi de la requête au backend /generate_roadmap...");
    ...
    try {
      ...
      debugPrint("Réponse reçue du backend /generate_roadmap (Status: ${response.statusCode})");
      ...
      if (response.statusCode == 200) {
        ...
        if (newRoadmap.isEmpty) {
          debugPrint("Le backend a retourné une roadmap vide.");
          throw ApiException("Le service de résumé n'a retourné aucun contenu.", statusCode: 500);
        }
        debugPrint("Nouvelle roadmap (passé) reçue du backend. Longueur: ${newRoadmap.length}");
        return newRoadmap;
      } else {
        final errorMsg = _extractApiError(response);
        debugPrint("Erreur API backend (${response.statusCode}) lors de la génération de la roadmap: $errorMsg");
        String detailError = errorMsg;
        try {
           final errorBody = jsonDecode(utf8.decode(response.bodyBytes));
           if (errorBody['detail'] != null) {
              detailError = errorBody['detail'].toString();
           }
        } catch (_) {}
        throw ApiException("Erreur lors de la génération du résumé: $detailError", statusCode: response.statusCode);
      }
    } on TimeoutException catch (e) {
       debugPrint("Timeout lors de l'appel à /generate_roadmap: $e");
       throw ApiConnectionException("Le délai pour générer le résumé a expiré (3 minutes).");
    } catch (e) {
       debugPrint("Erreur inattendue lors de l'appel à /generate_roadmap: $e");
  ...
  // ✅ CORRECTION : Logique de nettoyage améliorée
  static Chapter extractTitleAndContent(
    ...
    // 1. Corriger l'espacement avant les virgules
    chapterContent = chapterContent.replaceAll(RegExp(r'\s+,'), ',');
    // 2. Supprimer les tirets de dialogue en début de ligne
    chapterContent = chapterContent.replaceAll(RegExp(r'^\s*[-—]\s*', multiLine: true), '');
    // --- FIN ---

    String finalContent = chapterContent.trim(); // Re-trim après nettoyage
